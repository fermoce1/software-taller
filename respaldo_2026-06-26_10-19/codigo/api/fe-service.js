...
async function enviarApiExterna(cfg, payload) {
  ...
  return {
    clave: data.clave || data.Clave || data.key || '',
    consecutivo: data.consecutivo || data.Consecutivo || String(payload.consecutivo),
    estado: data.estado || data.Estado || 'aceptado',
    pdf_url: data.pdf_url || data.pdf || data.url_pdf || null,
    proveedor: data
  };
}
...
async function enviarAHacienda(db, facturaId) {
  ...
  if (!cfg.fe_api_url) {
    throw new Error('Configure la URL de la API de su proveedor en Configuración.');
  }
...
function listarFacturas(db, limit, q) {
  ...
  return db
    ...
    .all(like, like, like, like, like, like, like, like, lim);
}
